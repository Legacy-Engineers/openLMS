...
class AdminSettingsHomeView(AdminRequiredMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["recent_audit_logs"] = SystemAuditLog.objects.select_related("user")[
            :10
        ]
        return context
    ...
    def form_valid(self, form):
        try:
            ...
            if "company_logo" in form.cleaned_data:
                if (
                    form.cleaned_data["company_logo"] is None
                    and "company_logo-clear" not in self.request.POST
                ):
    ...
    if request.method == "POST":
        ...
        try:
            ...
            test_config = {
                ...
                "smtp_port": int(form_data.get("smtp_port", saved_config.smtp_port))
                if form_data.get("smtp_port")
                else saved_config.smtp_port,
                "smtp_username": form_data.get(
                    "smtp_username", saved_config.smtp_username
                ),
                "smtp_password": form_data.get(
                ...
                "reply_to_email": form_data.get(
                    "reply_to_email", saved_config.reply_to_email
                ),
                "test_email": form_data.get("test_email", saved_config.test_email),
                "use_ssl": form_data.get("use_ssl") == "on"
                if "use_ssl" in form_data
                else saved_config.use_ssl,
                "use_tls": form_data.get("use_tls") == "on"
                if "use_tls" in form_data
                else saved_config.use_tls,
            }

            if not test_config["test_email"]:
                return JsonResponse(
                    {"success": False, "error": "Please set a test email address first"}
                )
            ...
            try:
                ...
                if test_config["use_ssl"]:
                    server = smtplib.SMTP_SSL(
                        test_config["smtp_host"], test_config["smtp_port"]
                    )
                else:
                    server = smtplib.SMTP(
                        test_config["smtp_host"], test_config["smtp_port"]
            ...
            try:
                ...
                # Log the failed test
                test_email = (
                    form_data.get("test_email", saved_config.test_email)
                    if "form_data" in locals()
                    else saved_config.test_email
                )
                SystemAuditLog.objects.create(
                    user=request.user,
                    action_type="email_test",
...
class EmailTemplateUpdateView(AdminRequiredMixin, UpdateView):
    ...
    def form_valid(self, form):
        ...
        messages.success(
            self.request,
            f'Email template "{form.instance.get_template_type_display()}" updated successfully!',
        )
        return super().form_valid(form)
...
class AuditLogView(AdminRequiredMixin, ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["models"] = SystemAuditLog.objects.values_list(
    ...
    if request.method == "POST":
        try:
            ...
            SystemAuditLog.objects.create(
                ...
                changes={"is_active": user.is_active, "username": user.username},
                ip_address=request.META.get("REMOTE_ADDR"),
                user_agent=request.META.get("HTTP_USER_AGENT", ""),
            )
...
class PaymentMethodCreateView(AdminRequiredMixin, CreateView):
    ...
    def form_valid(self, form):
        ...
        messages.success(
            self.request, f'Payment method "{form.instance.name}" created successfully'
        )
...
class PaymentMethodUpdateView(AdminRequiredMixin, UpdateView):
    ...
    def form_valid(self, form):
        ...
        response = super().form_valid(form)
        messages.success(
            self.request, f'Payment method "{form.instance.name}" updated successfully'
        )
...
class PaymentMethodDeleteView(AdminRequiredMixin, DeleteView):
    ...
    def delete(self, request, *args, **kwargs):
        ...
        messages.success(
            request, f'Payment method "{payment_method.name}" deleted successfully'
        )
